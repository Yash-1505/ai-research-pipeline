...
export default function MonthlyPage() {
  ...
  const items = index?.monthly ?? [];

  if (items.length === 0) {
    return (
      <EmptyState message="No monthly reports yet. They're generated on the 5th of each month." />
    );
  }
      ...
      <main>
        {!selectedFile ? (
          <Card className="flex items-center justify-center h-64 text-slate-600">
            <p className="text-sm">← Select a month to read the report</p>
          </Card>
        ) : digestLoading ? (
